...
router.post('/bestellungen/:number/erneut', requireAuth, (req, res) => {
  const order = db.prepare('SELECT * FROM orders WHERE order_number=? AND user_id=?').get(req.params.number, req.session.userId);
  ...
  for (const item of items) {
    ...
    const product = db.prepare('SELECT id FROM products WHERE id=? AND active=1').get(item.product_id);
    if (!product) continue;
...
router.get('/bestellungen/:number/rechnung.pdf', requireAuth, (req, res) => {
  const order = db.prepare('SELECT * FROM orders WHERE order_number=? AND user_id=?').get(req.params.number, req.session.userId);
