...
router.post('/registrieren', async (req, res) => {
  try {
    ...
    try {
      const discountSetting = await db.prepare("SELECT value FROM settings WHERE key='welcome_discount_percent'").get();
      ...
      if (discountPercent > 0) {
        ...
        await db.prepare('INSERT INTO coupons (code, type, value, max_uses, user_id, expires_at) VALUES (?,?,?,?,?,?)')
