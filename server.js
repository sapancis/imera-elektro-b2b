...
app.use(helmet({
  contentSecurityPolicy: {
    directives: {
      ...
      connectSrc: ["'self'", 'https://www.google-analytics.com', 'https://region1.google-analytics.com', 'https://*.tawk.to', 'wss://*.tawk.to'],
...
app.get('/__migrate-catalog', async (req, res) => {
  ...
  try {
    const result = await require('./database/migrate-catalog')({
      force: req.query.force === '1',
      rebuildAllTiers: req.query.tiers === '1',
    });
    const db = require('./database/db');
